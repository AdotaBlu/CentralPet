...
<title>Home</title>
<link rel="icon" type="image/x-icon" href="./resources/img/icone-centralpet.ico">
...
<body>
	...
	<div class="conteinerHome">
		<div id="primeira-secao" class="container-secao">
			<p class="img-logo-home"><%@ include file="/resources/img/img-logo-home.svg"%></p>
			<p>
				<b>Adote</b> e ajude um 
				animalzinho a encontrar 
				novo lar a ele.
			</p>
		</div>
		
		<div id="segunda-secao" class="container-secao">
		<div>
								...
								<div class="info-bottom">
									<div class="pet-info-box">
										...
										<c:if test="${pet.idade > 1}">
											<span class="value"><c:out value='${pet.idade}'></c:out>
												anos</span>
										</c:if>
										<c:if test="${pet.idade == 1}">
									...
									<div class="pet-info-box">
										<span class="label">Porte: </span> <span class="value"><c:out value='${pet.portePet}'></c:out></span>
									</div>
									<div class="pet-info-box">
										<span class="label">Pelagem: </span> <span class="value"><c:out	value='${pet.pelagemPet}'></c:out></span>
									</div>
									<div class="pet-info-box">
										<span class="label">Peso: </span> <span class="value"><c:out value='${pet.peso}'></c:out>kg</span>
									</div>
								</div>
								<form method="post" action="mostrar-perfil-pet"
									enctype="form-data">
									<div class="btn-card">
		...
		<div id="terceira-secao" class="container-secao">
			<div>
				<p><b>Dê o primeiro passo para encontrar seu novo amigo	peludo! 
				Nesta etapa, explore os pets disponíveis para adoção. Veja suas 
				fotos, leia suas histórias e descubra qual deles toca seu coração.</p></b> 
			</div>
			
			<div>
				<b><p>Foram adotados através do central pet até agora.</b></p>
			</div>
			

		</div>
			
			<div id="quarta-secao" class="container-secao">
					...
					<c:forEach var="ong" items="${ongs}" varStatus="loop2">
						<c:if test="${loop2.index < 3}">
							<div class="ong-card">
								<div class="foto-pet">
									<img class="foto-pet"
										src="<c:out value='${ong.urlFoto()}'></c:out>" alt="foto-pet" width="220" height="170">
								</div>
								<div class="info-card">
									<div class="info-top">
		...
		<div id="quinta-secao" class="container-secao">
			<p>Essa são as ONGs que estão em destaque, deêm uma olhada no
				perfil delas para poder achar o seu novo amiguinho, e dar um novo
				lar e uma nova família para eles.</p>
		</div>
	...
	<main>
		...
		<br> <br> <br> <br> <br> <br> <br>
		<br> <br> 
		<a href="<%=request.getContextPath()%>/novo-tutor">CadastrarTutor</a> 
		...
		<a href="<%=request.getContextPath()%>/mostrar-perfil-ong">PerfilONG</a> 
		<a href="<%=request.getContextPath()%>/novo-pet">CadastrarPet</a> 
		<a href="<%=request.getContextPath()%>/editar-pet">Editar Pet</a>
		<a href="<%=request.getContextPath()%>/mostrar-perfil-pet">PerfilPet</a> 
		<a href="<%=request.getContextPath()%>/mostrar-cards-pets">PetsCadastrados</a> 
		<a href="<%=request.getContextPath()%>/mostrar-cards-ongs">ONGs
			Cadastradas</a>
			 <a
			href="<%=request.getContextPath()%>/mostrar-selecao-cadastro">Seleção
			Cadastro</a> 
